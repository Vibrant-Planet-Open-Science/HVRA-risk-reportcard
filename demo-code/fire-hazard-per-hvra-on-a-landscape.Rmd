---
title: "Fire hazard for `r params$hvra_name` in the `r stringr::str_to_title(params$landscape_name)` Landscape"
...
$~$
Here is the location of the HVRA on the landscape (area of interest).
$~$
...
$~$
Here is a map, histogram, and table summarizing the 10-year cumulative burn probability within the footprint of the hvra (resource) of interest, which ranges from `r min.bp.hvra*100` % to `r max.bp.hvra*100` %, with a mean value of `r round(hvra_10yr_burn_prob, digits = 2)*100` %.
$~$
...
bp_area_cropped  %>%
  kbl(caption = "Predicted wildfire exposure for the hvra, base on 10-year burn probability and broken into risk ranges, and expressed as acres and as a proportion of the total",  
      col.names = c("Range", "Acres", "Percent of Acres (%)"), format="pandoc", booktabs=T, format.args = list(big.mark = ","))
...
## Analysis of the expected fire behavior, expressed as flame length probabilities, within the hvra
$~$
...
## Analysis of the potential change in hvra value if a fire occurs
$~$
This is the expected response of the hvra to predicted flame lengths, if a fire were to occur. It is the proportional change, not percent change.
$~$
...
cond_loss_area  %>%
  kbl(caption = "Predicted proportional change in hvra value, if a fire were to occur (conditional loss/gain)",  
      col.names = c("Range", "Acres", "Percent of Acres (%)"), format="pandoc", booktabs=T, format.args = list(big.mark = ","))
...
## Analysis of the expected change in hvra value (fire hazard) in a "no action" scenario
$~$
This fire hazard analysis includes a) expected fire behavior (flame lengths) if a burn occurs, b) 10-year probability of a burn occurring, and c) expected hvra response to fire behavior (response functions) to get to an expected 10-year change in hvra value.
$~$
...
## Summary
$~$
Across the whole landscape, we expect the `r params$hvra_name` hvra to `r ifelse(test = hvra_value_change > 0, yes = "increase in", no = ifelse(test = hvra_value_change == 0, yes = "stay the same", no = "decrease in"))` value over the next 10 years.
Its 10-year average burn probability is `r round(hvra_10yr_burn_prob, digits = 3)*100` %.
Its potential change in value if it burns, on average, would be `r round(hvra_potential_value_change, digits = 3)*100` %.
Its response to fire hazard (which factors in the 10-year burn probability, the expected flame lengths if it burns, and the expected species response to those flame lengths) if we do nothing will be `r round(hvra_value_change, digits = 3)*100` % change in value.
